...
class RefusalAnalysisScorer(BaseScorer):
    ...
    Refusal details are returned via ``Score.details``.
    """
    ...
    async def score(self, result: AttackResult) -> Score:
        ...
        from agent_inject.detection import detect_compliance, detect_refusal

        output = result.raw_output
        has_refusal = detect_refusal(output)
        has_compliance = detect_compliance(output)
        ...
        if refusal_explanation:
            details["refusal_explanation"] = refusal_explanation
